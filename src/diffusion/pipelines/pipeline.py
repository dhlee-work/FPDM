...
@dataclass
class FPDM_DiffusionPipelineOutput(BaseOutput):
    images: Union[List[PIL.Image.Image], np.ndarray]
    nsfw_content_detected: Optional[List[bool]]


class FPDM_DiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if do_classifier_free_guidance:
            ...
            pose_cond = torch.cat([neg_pos_f, pose_cond, pose_cond], dim=0)
        ...
        if do_classifier_free_guidance:
            ...
            feature_f = torch.cat([neg_feature_f, feature_f, feature_f], dim=0)

            # target global feature
            if fusion_img_embed is not None:
                neg_prior_embed = torch.zeros(prior_embed.shape).to(device)  # , dtype=torch.float16
                prior_embed = torch.cat([neg_prior_embed, neg_prior_embed, prior_embed], dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # perform guidance
                if do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_src, noise_pred_global = noise_pred.chunk(3)
                    # noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_src - noise_pred_uncond) + \
                    #              guidance_scale * (noise_pred_global - noise_pred_src)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_src - noise_pred_uncond) + \
                                 guidance_scale * (noise_pred_global - noise_pred_src)
        ...
        return FPDM_DiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)
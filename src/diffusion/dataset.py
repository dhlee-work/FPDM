...
class FPDM_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        t_keypoint = self.PK.trans_keypoins(t_keypoint, self.model_img_size, self.kpt_param)
        s_pose_path = s_img_path.replace('img', 'pose').replace('.jpg', '.txt')
        s_keypoint = np.loadtxt(s_pose_path)
        s_keypoint = self.PK.trans_keypoins(s_keypoint, self.model_img_size, self.kpt_param)
